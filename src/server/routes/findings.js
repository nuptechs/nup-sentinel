...
export function createFindingRoutes(services) {
  ...
  // POST /api/findings/:id/open-pr — Open a GitHub Pull Request applying
  // the AI-generated correction. Closes MATRIZ-COMPETITIVA.md eixo J.
  //
  // Requires:
  //   - The finding must already have `correction.suggestedFix.files[]`
  //     populated (run /:id/correct first).
  //   - SENTINEL_GITHUB_TOKEN env configured (or 503).
  //
  // Body:
  //   {
  //     "repoOwner": "nuptechs",
  //     "repoName":  "easynup",
  //     "branchName": "sentinel/finding-<id>-<slug>",
  //     "title":  "...",          // optional, derived from finding
  //     "body":   "...",          // optional, derived from finding
  //     "baseBranch": "main",     // optional, defaults to repo default
  //     "commitMessage": "..."    // optional
  //   }
  router.post('/:id/open-pr', asyncHandler(async (req, res) => {
    const codeChange = services?.codeChange;
    if (!codeChange?.isConfigured?.()) {
      return res.status(503).json({
        success: false,
        error: 'code_change_unavailable',
        message: 'SENTINEL_GITHUB_TOKEN is not configured',
      });
    }

    const finding = await services.findings.get(req.params.id);
    const fix = finding?.correction?.suggestedFix || finding?.diagnosis?.suggestedFix;
    if (!fix || !Array.isArray(fix.files) || fix.files.length === 0) {
      throw new ValidationError(
        'finding has no correction.suggestedFix.files — run POST /:id/correct first',
      );
    }
    ...
    if (!repoOwner) throw new ValidationError('repoOwner (string) is required');
    if (!repoName) throw new ValidationError('repoName (string) is required');

    const branchName =
      typeof body.branchName === 'string' && body.branchName.trim()
        ? body.branchName.trim()
        : `sentinel/finding-${finding.id}`;

    const title =
      typeof body.title === 'string' && body.title.trim()
        ? body.title
        : `Sentinel: fix ${finding.type}/${finding.subtype || finding.severity} — ${finding.title}`;

    const prBody =
      typeof body.body === 'string' && body.body.length > 0
        ? body.body
        : buildDefaultPrBody(finding, fix);

    let result;
    try {
      result = await codeChange.openPullRequest({
        repoOwner,
        repoName,
      ...
      const code = err?.code || (err?.statusCode === 422 ? 'github_validation' : 'pr_failed');
  ...
  function buildDefaultPrBody(finding, fix) {
    const lines = [
      `**Auto-generated by [nup-sentinel](https://sentinel.nuptechs.com)**`,
      ``,
      `### Finding`,
      `- **id:** \`${finding.id}\``,
      `- **type:** ${finding.type}${finding.subtype ? ` / ${finding.subtype}` : ''}`,
      `- **severity:** ${finding.severity || 'medium'}`,
      finding.symbolRef?.identifier ? `- **symbol:** \`${finding.symbolRef.identifier}\`` : null,
      ``,
      `### Description`,
      finding.description || '_no description_',
      ``,
      `### Diagnosis`,
      finding.diagnosis?.rootCause || finding.diagnosis?.explanation || '_no diagnosis_',
      ``,
      `### Suggested fix`,
      fix.summary || '_(see commit diff)_',
    ];
    if (fix.testSuggestion) {
      lines.push('', '### Tests', fix.testSuggestion);
    }
